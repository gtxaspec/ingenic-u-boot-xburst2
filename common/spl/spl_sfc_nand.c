...
static void sfc_controler_init(void)
{
	unsigned int tmp;
#ifndef CONFIG_FPGA
	clk_set_rate(SFC0, CONFIG_SFC_NAND_RATE);
	clk_set_rate(SFC1, CONFIG_SFC_NAND_RATE);
#endif
...
read_oob:
	 ...
	 * DS	 DS35Q2GA		 DID : 0x72
	 */
	if(curr_device->device_id == 0x20 || curr_device->device_id == 0x22 || curr_device->device_id == 0x24 || curr_device->device_id == 0x72)
		column |= (((page >> 6) & 1) << 12);
