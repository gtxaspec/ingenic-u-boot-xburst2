...
extern struct dwc2_core_regs *regs;
/******************************************************************************
...
static int do_usb(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (strncmp(argv[1], "setport", 4) == 0) {
                if (argc == 3){
                        usb_stor_curr_port = (int)simple_strtoul(argv[2], NULL, 10);

                        if(usb_stor_curr_port < 0 || usb_stor_curr_port > 2)
                        {
                                printf("error!! usb port%d,set deafult port0\n",usb_stor_curr_port);
                                usb_stor_curr_port=0;
                        }
                        printf("set USB port%d \n\n", usb_stor_curr_port);
                }else{
                        printf("input format: usb setport 0/1/2\n ");
                }
		return 0;
	}
