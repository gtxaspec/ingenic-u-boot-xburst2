...
static void fill_in_params_ddr2(struct ddr_params *p)
{
	int tmp;
...
static void ddrc_params_creator_ddr2(struct ddrc_reg *ddrc, struct ddr_params *p)
{
    ...
    if (ddrc->timing1.b.tWL > 0) {
        ...
        out_error("DDR_WL too small!! please check %s, %d\n", __FILE__, __func__);
        assert(1);
    }
    ...
    if (ddrc->timing2.b.tRL > 0) {
        ...
        out_error("DDR_RL too small!! please check %s, %d\n", __FILE__, __func__);
        assert(1);
    }
	...
	tmp = ps2cycle_ceil(p->private_params.ddr3_params.tXSNR,1);
	ASSERT_MASK(tmp,8);
