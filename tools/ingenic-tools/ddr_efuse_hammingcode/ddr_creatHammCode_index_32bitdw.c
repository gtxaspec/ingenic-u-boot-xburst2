...
const int A1_DDR_ODT_REMAP[8] = {0,1,2,3,5,7,0xB,0x1F};
const int A1_DDR_DRVCMD_REMAP[8] = {1,2,4,6,8,0xB,0xE,0x1F};
const int A1_DDR_DRVDATA_REMAP[8] = {1,2,4,6,7,0x14,0x18,0x1F};
const int A1_DDR_DQ_REMAP[8] = {7,12,14,16,18,20,22,24};
...
int set_ddr_data(int data[])
{
    ...
    for(i, j = 0; j < 4; j++, i++)
		data[i] = (ddr3_data.index4 >> j) & 0x1;
	for(i, j = 0; j < get_param_width(ddr3_data.index4); j++, i++)
		data[i] = (ddr3_data.data4 >> j) & 0x1;
...
int main(int argc, char* argv[])
{
	...
	int dataBits = A1_EFUSE_PARAM_DATA_NUBBIT_32BIT + 1;
	int numParityBits = check_ParityBits(dataBits);
	int hammingCode[dataBits + numParityBits + 1];

	int decodedData = 0;
	int i = 0, ret = 0;
	int data[A1_EFUSE_PARAM_DATA_NUBBIT_32BIT + 1] = {0};
	...
	printf("32bit index data %d is: ",dataBits);
