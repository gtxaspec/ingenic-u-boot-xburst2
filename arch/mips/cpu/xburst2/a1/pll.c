...
extern unsigned short get_chip_type_from_efuse(void);
#endif
...
static void pll_set(int pll,int freq)
{
	...
	case APLL:
		/* Init APLL */
#ifdef CONFIG_A1ALL
		cppcr.d32 = pddr_params->config_sys_apll_mnod;
#else
	...
	case MPLL:
		/* Init MPLL */
#ifdef CONFIG_A1ALL
	cppcr.d32 = pddr_params->config_sys_mpll_mnod;
#else
	...
	case EPLL:
		/* Init EPLL */
#ifdef CONFIG_A1ALL
		cppcr.d32 = pddr_params->config_sys_epll_mnod;
#else
		cppcr.d32 = regvalue;
#endif
...
static void cpccr_init(void)
{
	...
	cpccr = (cpm_inl(CPM_CPCCR) & (0xff << 24))
		| (pddr_params->config_sys_cpccr_sel & ~(0xff << 24))
		| (7 << 20);

	cpm_outl(cpccr,CPM_CPCCR);

	ahb1_cpccr = (cpm_inl(CPM_AHB1CDR) & 0x3)
		| (pddr_params->config_sys_ahb1cpccr_sel & ~0x3)
		| (0x1 << 8);
...
#ifdef CONFIG_A1ALL
	cpccr = (pddr_params->config_sys_cpccr_sel & (0xff << 24)) | (cpm_inl(CPM_CPCCR) & ~(0xff << 24));
	cpm_outl(cpccr,CPM_CPCCR);
	ahb1_cpccr = (pddr_params->config_sys_ahb1cpccr_sel & (0x3)) | (cpm_inl(CPM_AHB1CDR) & (~0x3));
#else
...
static void final_fill_div(int cpll, int pclk)
{
	...
	/*DDRDIV*/
#ifdef CONFIG_A1ALL
	gd->arch.gi->ddr_div = Periph_pll_freq/pddr_params->config_sys_mem_freq;
	/*cdiv*/
	pll_cfg.cdiv = cpu_pll_freq/pddr_params->config_sys_apll_freq;
#else
...
#ifdef CONFIG_A1ALL
	pll_cfg.mpll_freq = pddr_params->config_sys_mpll_freq > 0 ? pddr_params->config_sys_mpll_freq : 0;
#else
...
#else /* CONFIG_SLT */
#ifdef CONFIG_A1ALL
	pll_cfg.apll_freq = pddr_params->config_sys_apll_freq > 0 ? pddr_params->config_sys_apll_freq : 0;
#else
...
#endif /* CONFIG_SLT */
#ifdef CONFIG_A1ALL
	if (!pddr_params->config_sys_apll_freq && !pddr_params->config_sys_mem_freq) {
		error("cpufreq = %d and ddrfreq = %d can not be zero, check board config\n",
				pddr_params->config_sys_apll_freq, pddr_params->config_sys_mem_freq);
		asm volatile ("wait\n\t");
	}
...
#ifdef CONFIG_A1ALL
			pll_cfg.mpll_freq = align_pll(pll_cfg.mpll_freq, pddr_params->config_sys_mem_freq);
			pll_cfg.apll_freq = align_pll(pll_cfg.apll_freq, pddr_params->config_sys_apll_freq);
#else
...
#ifdef CONFIG_A1ALL
			pll_cfg.apll_freq = align_pll(pll_cfg.apll_freq, pddr_params->config_sys_mem_freq);
			pll_cfg.mpll_freq = align_pll(pll_cfg.mpll_freq, pddr_params->config_sys_apll_freq);
