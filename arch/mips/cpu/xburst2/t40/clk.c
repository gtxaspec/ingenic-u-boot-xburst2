...
static unsigned int get_msc_rate(unsigned int xcdr)
{
	...
	case 2:
		...
		break;
	case 3:
		ret = CONFIG_SYS_EXTAL / (((mscxcdr & 0xff) + 1) * 2);
		break;
	default:
		break;
	}
...
{
	unsigned int cdr;
	unsigned int pll_rate, sel_src, j;
	struct cgu *cgu = NULL;
	unsigned regval = 0;
    ...
    {
        if (rate <= 400000) // 如果小于400KHz就使用EXCLK
            sel_src = EXCLK;

        regval &= ~(3 << 30);
        for (j = 0; j < 4; j++)
        {
            if (sel_src == cgu->sel[j] && cgu->en == 1) // 配置时钟源
            {
                regval |= j << cgu->sel_bit;

                if (sel_src == EXCLK) // 如果是EXCLK 需要需要使能MSC_EXCLKEN
                    regval |= (1 << 21);
                else
                    regval &= ~(1 << 21);
                break;
