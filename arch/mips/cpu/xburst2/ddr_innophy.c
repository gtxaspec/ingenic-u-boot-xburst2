...
#ifdef CONFIG_A1ALL
	unsigned short chip = get_chip_type_from_efuse();
	if (chip == 0x1111){ //A1N
                odt_pd = 1;
                odt_pu = 0;
                drvcmd = 0xe;
...
#define A1X_DDR_VERSION 2

                printf("A1X DDR V%d\n",A1X_DDR_VERSION);
#if(A1X_DDR_VERSION == 2)
                odt_pd = 5;
                odt_pu = 11;
                drvcmd = 0x1e;
...
#else //if open ZQCL, use this parameters will not enter uboot cmd line.
                odt_pd = 7;
                odt_pu = 1;
                drvcmd = 0xe;

                drval_pd = 0xe; drval_pu = 0x01;
                drvah_pd = 0xe; drvah_pu = 0x01;
                drvbl_pd = 0x19; drvbl_pu = 0x1f;
                drvbh_pd = 0x16; drvbh_pu = 0x03;
                ...
                ddr_test_phy_dq_bit_set(0, -1, 14); //set a channel dq value 14.
                ddr_test_phy_dq_bit_set(1, -1, 14); //set b channel dq value 14.
#endif
...
static void ddrp_hardware_calibration(void)
{
	...
	if (pddr_params->config_ddr_dw32 == 1) {
			...
			printf("timeout is 0x%x, val = 0x%x\n", timeout, val);
	}
#else
#if CONFIG_DDR_DW32
	do
