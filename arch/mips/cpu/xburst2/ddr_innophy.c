...
ddr3_param_t t40_ddr3_param = {
    .version = "T40@V20240304",
    .params = {
        [odt_pd] = 3,
        [odt_pu] = 3,
        [drvcmd_pd] = 0xf,
...
static void ddrp_pll_init(void)
{
	 ...
	 * fpll_1xclk = fpll_4xclk / 4 = fpll_refclk;
...
static void ddrp_software_writeleveling_tx(ddr3_param_t *ddr3_param)
{
    ...
    writel(ddr3_param->params[dqsA], 0xb30114a4); //A_DQS0
    writel(ddr3_param->params[dqsA], 0xb30114d0); //A_DQS1
    writel(ddr3_param->params[dqsB], 0xb30116a4); //B_DQS0
    writel(ddr3_param->params[dqsB], 0xb30116d0); //B_DQS1

    writel(ddr3_param->params[dqsA], 0xb30114a8); //A_DQSB0
    writel(ddr3_param->params[dqsA], 0xb30114d4); //A_DQSB1
    writel(ddr3_param->params[dqsB], 0xb30116a8); //B_DQSB0
    writel(ddr3_param->params[dqsB], 0xb30116d4); //B_DQSB1
...
static void ddr_innophy_set_skew(ddr3_param_t *ddr3_param){
	u32 DQxOFFSET_rx[] = {0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x2c, 0x2d, 0x2e, 0x2f, 0x30, 0x31, 0x32, 0x33}; // DQxxx RX寄存器偏移
	u32 DQxOFFSET_tx[] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13}; // DQxxx TX寄存器偏移
	u32 dqs_addr_a = 0x120, dqs_addr_b = 0x1a0;
	u32 i;
	u32 vref_cha = 0x7f, vref_chb = 0x7f;
	u32 dqs0_rx = 0x18, dqs1_rx = 0x18;
	u32 dqx0_rx = 0x11, dqx1_rx = 0x11;
	u32 dqs0_tx = 0x07, dqs1_tx = 0x07;
	u32 dqx0_tx = 0x0e, dqx1_tx = 0x0e;

#ifdef CONFIG_T40XP
	vref_cha = ddr3_param->params[vref]; vref_chb = ddr3_param->params[vref];

	dqs0_rx = 0x18; dqs1_rx = 0x18;
	dqx0_rx = 0x11; dqx1_rx = 0x11;
	dqs0_tx = 0x07; dqs1_tx = 0x07;
	dqx0_tx = 0x0e; dqx1_tx = 0x0e;
	printf("T40XP ddr phy skew setting\n");
	return ;
#elif defined CONFIG_T40N
	vref_cha = 0x7f; vref_chb = 0x7f;

	dqs0_rx = 0x18; dqs1_rx = 0x18;
	dqx0_rx = 0x11; dqx1_rx = 0x11;
	dqs0_tx = 0x07; dqs1_tx = 0x07;
	dqx0_tx = 0x0e; dqx1_tx = 0x0e;
	printf("T40N ddr phy skew setting\n");
...
#if 1
	/* vref */
	writel(vref_cha, 0xb3011000 + 0xd7 * 4);
	writel(vref_cha, 0xb3011000 + 0xd8 * 4);
	writel(vref_chb, 0xb3011000 + 0xf7 * 4);
	writel(vref_chb, 0xb3011000 + 0xf8 * 4);
#endif

#if 1
	/* rx */
	writel(dqs0_rx, 0xb3011000 + (dqs_addr_a + 0x29) * 4);
	writel(dqs1_rx, 0xb3011000 + (dqs_addr_a + 0x34) * 4);

	writel(dqs0_rx, 0xb3011000 + (dqs_addr_b + 0x29) * 4);
	writel(dqs1_rx, 0xb3011000 + (dqs_addr_b + 0x34) * 4);

	for (i = 0; i < 8; i++)
	{
		writel(dqx0_rx, 0xb3011000 + (dqs_addr_a + DQxOFFSET_rx[i]) * 4);
		writel(dqx0_rx, 0xb3011000 + (dqs_addr_b + DQxOFFSET_rx[i]) * 4);
	}
	for (i = 8; i < 16; i++)
	{
		writel(dqx1_rx, 0xb3011000 + (dqs_addr_a + DQxOFFSET_rx[i]) * 4);
		writel(dqx1_rx, 0xb3011000 + (dqs_addr_b + DQxOFFSET_rx[i]) * 4);
	}
#endif
#if 1
	/* tx */
	for (i = 0; i <= 0x1e; i++)
	{
		ddr_writel(dqs0_tx, 0xb3011000 + (0x100 + i) * 4); /// cmd
	}
	writel(dqs0_tx, 0xb3011000 + (dqs_addr_a + 0x00) * 4); // dm0
	writel(dqs0_tx, 0xb3011000 + (dqs_addr_a + 0x09) * 4); // dqs0
	writel(dqs0_tx, 0xb3011000 + (dqs_addr_a + 0x0a) * 4); // dqsb0
	writel(dqs1_tx, 0xb3011000 + (dqs_addr_a + 0x0b) * 4); // dm1
	writel(dqs1_tx, 0xb3011000 + (dqs_addr_a + 0x14) * 4); // dqs1
	writel(dqs1_tx, 0xb3011000 + (dqs_addr_a + 0x15) * 4); // dqsb1

	writel(dqs0_tx, 0xb3011000 + (dqs_addr_b + 0x00) * 4); // dm0
	writel(dqs0_tx, 0xb3011000 + (dqs_addr_b + 0x09) * 4); // dqs0
	writel(dqs0_tx, 0xb3011000 + (dqs_addr_b + 0x0a) * 4); // dqsb0
	writel(dqs1_tx, 0xb3011000 + (dqs_addr_b + 0x0b) * 4); // dm1
	writel(dqs1_tx, 0xb3011000 + (dqs_addr_b + 0x14) * 4); // dqs1
	writel(dqs1_tx, 0xb3011000 + (dqs_addr_b + 0x15) * 4); // dqsb1

	for (i = 0; i < 8; i++)
	{
		writel(dqx0_tx, 0xb3011000 + (dqs_addr_a + DQxOFFSET_tx[i]) * 4);
		writel(dqx0_tx, 0xb3011000 + (dqs_addr_b + DQxOFFSET_tx[i]) * 4);
	}
	for (i = 8; i < 16; i++)
	{
		writel(dqx1_tx, 0xb3011000 + (dqs_addr_a + DQxOFFSET_tx[i]) * 4);
		writel(dqx1_tx, 0xb3011000 + (dqs_addr_b + DQxOFFSET_tx[i]) * 4);
	}
#endif
}

static void ddr_phy_cfg_driver_odt(ddr3_param_t *ddr3_param)
{
	...
	/* ck */
	ddr_phyreg_set_range(0xb2, 0, 5, 0xb); //pull down
	ddr_phyreg_set_range(0xb3, 0, 5, 0xb);//pull up
	...
	u32 dq_ds_ad = ddr3_param->params[drvalah_pd];
	u32 dq_ds_au = ddr3_param->params[drvalah_pu];
	u32 dq_ds_bd = ddr3_param->params[drvblbh_pd];
	u32 dq_ds_bu = ddr3_param->params[drvblbh_pu];

	ddr_phyreg_set_range(0xc2, 0, 5, dq_ds_ad);//pull down
	ddr_phyreg_set_range(0xc3, 0, 5, dq_ds_au);//pull up
	ddr_phyreg_set_range(0xd2, 0, 5, dq_ds_ad);
	ddr_phyreg_set_range(0xd3, 0, 5, dq_ds_au);

	ddr_phyreg_set_range(0xe2, 0, 5, dq_ds_bd);
	ddr_phyreg_set_range(0xe3, 0, 5, dq_ds_bu);
	ddr_phyreg_set_range(0xf2, 0, 5, dq_ds_bd);
	ddr_phyreg_set_range(0xf3, 0, 5, dq_ds_bu);

#elif defined (CONFIG_T40XP)
	/* cmd */
	ddr_phyreg_set_range(0xb0, 0, 5, ddr3_param->params[drvcmd_pd]);
	ddr_phyreg_set_range(0xb1, 0, 5, ddr3_param->params[drvcmd_pu]);
	/* ck */
	ddr_phyreg_set_range(0xb2, 0, 5, ddr3_param->params[drvcmdck_pd]); //pull down
	ddr_phyreg_set_range(0xb3, 0, 5, ddr3_param->params[drvcmdck_pu]);//pull up

	/* DQ ODT config */
	u32 dq_odt_pd = ddr3_param->params[odt_pd];
	u32 dq_odt_pu = ddr3_param->params[odt_pu];

	/* Channel A */
	ddr_phyreg_set_range(0xc0, 0, 5, dq_odt_pd);
	ddr_phyreg_set_range(0xc1, 0, 5, dq_odt_pd);
	ddr_phyreg_set_range(0xd0, 0, 5, dq_odt_pd);
	ddr_phyreg_set_range(0xd1, 0, 5, dq_odt_pd);
	/* Channel B */
	ddr_phyreg_set_range(0xe0, 0, 5, dq_odt_pu);
	ddr_phyreg_set_range(0xe1, 0, 5, dq_odt_pu);
	ddr_phyreg_set_range(0xf0, 0, 5, dq_odt_pu);
	ddr_phyreg_set_range(0xf1, 0, 5, dq_odt_pu);
	...
	#elif defined (CONFIG_T40N)
	u32 odt_pd    = 0x5, odt_pu    = 0x5;
	u32 drvcmd_pd = 0x4, drvcmd_pu = 0x4;
	u32 drvclk_pd = 0x9, drvclk_pu = 0x9;
	u32 drval_pd  = 0x3, drval_pu  = 0x3;
	u32 drvah_pd  = 0x3, drvah_pu  = 0x3;

	//driver cmd and ck
	//cmd
	ddr_phyreg_set_range(0xb0, 0, 5, drvcmd_pd); //0x4
	ddr_phyreg_set_range(0xb1, 0, 5, drvcmd_pu);
	//ck
	ddr_phyreg_set_range(0xb2, 0, 5, drvclk_pd); //pull down,0x5
	ddr_phyreg_set_range(0xb3, 0, 5, drvclk_pu);//pull up
	//dq ODT
	//ch A
	ddr_phyreg_set_range(0xc0, 0, 5, odt_pd);
	ddr_phyreg_set_range(0xc1, 0, 5, odt_pu);
	ddr_phyreg_set_range(0xd0, 0, 5, odt_pd);
	ddr_phyreg_set_range(0xd1, 0, 5, odt_pu);
	//chb B
	ddr_phyreg_set_range(0xe0, 0, 5, odt_pd);
	ddr_phyreg_set_range(0xe1, 0, 5, odt_pu);
	ddr_phyreg_set_range(0xf0, 0, 5, odt_pd);
	ddr_phyreg_set_range(0xf1, 0, 5, odt_pu);
	// driver strength
	ddr_phyreg_set_range(0xc2, 0, 5, drval_pd);//pull down
	ddr_phyreg_set_range(0xc3, 0, 5, drval_pu);//pull up
	ddr_phyreg_set_range(0xd2, 0, 5, drvah_pd);
	ddr_phyreg_set_range(0xd3, 0, 5, drvah_pu);

	ddr_phyreg_set_range(0xe2, 0, 5, drval_pd);
	ddr_phyreg_set_range(0xe3, 0, 5, drval_pu);
	ddr_phyreg_set_range(0xf2, 0, 5, drvah_pd);
	ddr_phyreg_set_range(0xf3, 0, 5, drvah_pu);
...
static void ddr_phy_init(ddr3_param_t *ddr3_param)
{
	u32 idx;
	u32  val;
	/* bit[3]reset digital core; bit[2]reset analog logic; bit[0]Reset Initial status
	...
	val |= 0x0d;
	ddr_writel(val, DDRP_INNOPHY_INNO_PHY_RST);
	ddr_phy_cfg_driver_odt(ddr3_param);

#ifdef CONFIG_T40A
	writel(0xb, 0xb3011000 + (0x15)*4);//default 0x4 cmd
	writel(0x1, 0xb3011000 + (0x16)*4);//default 0x0 ck

	writel(0xb, 0xb3011000 + (0x54)*4);//default 0x4  byte0 dq dll
	writel(0xb, 0xb3011000 + (0x64)*4);//default 0x4  byte1 dq dll
	writel(0xb, 0xb3011000 + (0x84)*4);//default 0x4  byte2 dq dll
	writel(0xb, 0xb3011000 + (0x94)*4);//default 0x4  byte3 dq dll
...
#ifdef CONFIG_T40A
		ddr_writel(DDRC_LMR_START | DDRC_LMR_CMD_ZQCL_CS0, DDRC_LMR); //ZQC
		udelay(5);
#endif
#undef DDRC_LMR_MR
		break;
...
void sdram_init(void)
{
	...
	printf("odt_pd %x\nodt_pu %x\ndrvcmd_pd %x\ndrvcmd_pu %x\ndrvcmdck_pd %x\ndrvcmdck_pu %x\n",
					  ddr3_param_ptr->params[odt_pd],
					  ddr3_param_ptr->params[odt_pu],
					  ddr3_param_ptr->params[drvcmd_pd],
					  ddr3_param_ptr->params[drvcmd_pu],
					  ddr3_param_ptr->params[drvcmdck_pd],
					  ddr3_param_ptr->params[drvcmdck_pu]);
	printf("drvalah_pd %x\ndrvalah_pu %x\ndqA %x\nvref %x\nkgd_odt %x\nkgd_ds%x\ndrvblbh_pd %x\ndrvblbh_pu %x\ndqB %x\ndqsA %x\n\n",
					  ddr3_param_ptr->params[drvalah_pd],
					  ddr3_param_ptr->params[drvalah_pu],
					  ddr3_param_ptr->params[dqA],
					  ddr3_param_ptr->params[vref],
					  ddr3_param_ptr->params[kgd_odt],
					  ddr3_param_ptr->params[kgd_ds],
					  ddr3_param_ptr->params[drvblbh_pd],
					  ddr3_param_ptr->params[drvblbh_pu],
					  ddr3_param_ptr->params[dqB],
					  ddr3_param_ptr->params[dqsA]);
#endif
...
//	ddr_param_write(ddr3_param_ptr);
	...
	ddrp_hardware_calibration();
#endif
	//ddrp_software_writeleveling_tx(ddr3_param_ptr);
	...
	//ddr_phyreg_set_range(0x9, 7, 1, 1);
    	...
    	//ddr_phy_cfg_per_bit_de_skew();
   	//ddrp_software_writeleveling_rx();
	ddr_innophy_set_skew(ddr3_param_ptr);
