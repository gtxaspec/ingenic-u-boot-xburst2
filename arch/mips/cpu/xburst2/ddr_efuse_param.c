...
const unsigned char A1_DDR_PARAM_WIDTH[kgd_ds+1] = {
    5,//odt_pd
    5,//odt_pu
    5,//drvcmd_pd
    5,//drvcmd_pu
...
#ifndef DEBUG_DDR_EFUSE_PARAM
    if(index < odt_pd || index > kgd_ds){
        printf("index not support!\n");
        return;
    }else{
        ddr_data->params[index] = data;
        printf("get ddr3_data.index%d is 0x%x\n",index, data);
    }
#else
	switch(index)
	{
...
int get_param_width(int index)
{
    ...
    width = A1_DDR_PARAM_WIDTH[index];
    ...
    if(data[0])//index mode
    {
        ...
        printf("index mode\n");
         if(is16bitdw){//16bit dw
            ...
            for(j = 0; j < data0width; i++, j++)
                data_0 |= (data[i] & 0x1) << j;
            get_index_ddr(index_0, data_0,ddr_data);

            for(j = 0; j < A1_EFUSE_INDEX_MODE_WIDTH; i++, j++)
