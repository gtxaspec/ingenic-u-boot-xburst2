...
struct t41_ddr_params *pddr_params;
...
#define T40_EFUSE_PARAM_REG0 0xb3540244
#define T40_EFUSE_PARAM_REG1 0xb3540248

#define T40_EFUSE_PARAM_HAMM_NUBBIT_16BIT 52
#define T40_EFUSE_PARAM_DATA_NUBBIT_16BIT 44
#define T40_EFUSE_PARAM_INDEX_MODE_BIT 1
#define T40_EFUSE_PARAM_LAST_XOR_BIT 1

#define T40_EFUSE_PARAM_HAMM_NUBBIT_32BIT 64
#define T40_EFUSE_PARAM_DATA_NUBBIT_32BIT 56

#define T40_EFUSE_INDEX_MODE_WIDTH 4
#define DEBUG_DDR_EFUSE_PARAM
#define INDEX_DATABIT 7

const int T40_DDR_ODT_REMAP[16] = {0,1,2,3,3,5,6,7,8,9,0xA,0xB, 0xD,0x18,0x1B,0x1F};

const int T40_DDR_DRVCMD_REMAP[16] = {0x11,0x11,2,3,4,5,6,0x11,8,9,0xA,0xB,1,1,0xf, 0xf,0x18,0x1B,0x1F};
const int T40_DDR_DRVDATA_REMAP[16] = {0,0x11,2,3,4,5,6,7,8,9,0xA,0xB, 0x14,0x18,0x1B,0x1F};


const int T40_DDR_DQ_REMAP[16] = { 5,7,9,11,13,14,15,16,8,18,19,20,21,22,24,26};
const int T40_DDR_VREF_REMAP[8] = {118,128,138,148,158,168,178,0x7f};
...
void get_ddr_data(int data[], int numBits ,ddr3_param_t *ddr_data,bool is16bitdw)
{
    ...
    int index_1 = 0, data_1 = 0, datt41width = 0;
    ...
    {
//		memset(ddr_data->params, 0 ,sizeof(uint8_t)*(dqsA + 1)); //参数清0，dqs除外

        printf("index mode\n");
         if(is16bitdw){//16bit dw
            for(i = 1, j = 0; j < T40_EFUSE_INDEX_MODE_WIDTH; i++, j++)
                index_0 |= (data[i] & 0x1) << j;
            //data0width = get_param_width(index_0);
            allbits = T40_EFUSE_INDEX_MODE_WIDTH + data0width;
            for(i, j = 0; j < INDEX_DATABIT; i++, j++)
                data_0 |= (data[i] & 0x1) << j;
            get_index_ddr(index_0, data_0,ddr_data);

            for(i, j = 0; j < T40_EFUSE_INDEX_MODE_WIDTH; i++, j++)
                index_1 |= (data[i] & 0x1) << j;
            if(!index_1)
                goto END;

            //datt41width = get_param_width(index_1);
            allbits += T40_EFUSE_INDEX_MODE_WIDTH + datt41width;
            for(i, j = 0; j < datt41width; i++, j++)
                data_1 |= (data[i] & 0x1) << j;
            ...
            allbits += T40_EFUSE_INDEX_MODE_WIDTH + data4width;
            if(allbits > (T40_EFUSE_PARAM_HAMM_NUBBIT_16BIT - 4))
                return;
            ...
            //data0width = get_param_width(index_0);
			printf("index_0: %x\n",index_0);
            allbits = T40_EFUSE_INDEX_MODE_WIDTH + INDEX_DATABIT;
            for(i, j = 0; j < INDEX_DATABIT; i++, j++)
                data_0 |= (data[i] & 0x1) << j;
            get_index_ddr(index_0, data_0,ddr_data);
           ...
           //if(!index_4)
            ...
            allbits += T40_EFUSE_INDEX_MODE_WIDTH + INDEX_DATABIT;
            if(allbits > (T40_EFUSE_PARAM_HAMM_NUBBIT_32BIT - 4)){
#ifdef DEBUG_DDR_EFUSE_PARAM
                    printf("4 parms,allbits:%d,> %d\n",allbits,(T40_EFUSE_PARAM_HAMM_NUBBIT_32BIT - 4));
#endif
                    return;
            }
/*
            ...
            allbits += T40_EFUSE_INDEX_MODE_WIDTH + data5width;
...
*/
        ...
        ddr_data->params[odt_pd] = T40_DDR_ODT_REMAP[ddr_data->params[odt_pd]];
        ddr_data->params[odt_pu] = T40_DDR_ODT_REMAP[ddr_data->params[odt_pu]];
        ddr_data->params[drvcmd_pd] = T40_DDR_DRVCMD_REMAP[ddr_data->params[drvcmd_pd]];
        ddr_data->params[drvcmd_pu] = T40_DDR_DRVCMD_REMAP[ddr_data->params[drvcmd_pu]];
        ddr_data->params[drvcmdck_pd] = T40_DDR_DRVCMD_REMAP[ddr_data->params[drvcmdck_pd]];
        ddr_data->params[drvcmdck_pu] = T40_DDR_DRVCMD_REMAP[ddr_data->params[drvcmdck_pu]];
        ddr_data->params[drvalah_pd] = T40_DDR_DRVDATA_REMAP[ddr_data->params[drvalah_pd]];
        ddr_data->params[drvalah_pu] = T40_DDR_DRVDATA_REMAP[ddr_data->params[drvalah_pu]];
        ddr_data->params[drvblbh_pd] = T40_DDR_DRVDATA_REMAP[ddr_data->params[drvblbh_pd]];
        ddr_data->params[drvblbh_pu] = T40_DDR_DRVDATA_REMAP[ddr_data->params[drvblbh_pu]];
        ddr_data->params[dqA] = T40_DDR_DQ_REMAP[ddr_data->params[dqA]];
        ddr_data->params[dqB] = T40_DDR_DQ_REMAP[ddr_data->params[dqB]];
        ddr_data->params[vref] = T40_DDR_VREF_REMAP[ddr_data->params[vref]];
...
int set_ddr_data(int reg1,int reg2,uint8_t data[],int numBits,bool is16bitdw)
{
    ...
    if(true == is16bitdw)
        allbit = T40_EFUSE_PARAM_HAMM_NUBBIT_16BIT;
    else
        allbit = T40_EFUSE_PARAM_HAMM_NUBBIT_32BIT;
...
{
    int reg1 = 0,reg2 = 0,ret = 0,value0 = 0,value1 = 0, tmp;
	uint8_t data[T40_EFUSE_PARAM_HAMM_NUBBIT_32BIT] = {0};
	...
	/* Large end and small end conversion */
	value0 = ___swab32(readl(T40_EFUSE_PARAM_REG0));
	value1 = ___swab32(readl(T40_EFUSE_PARAM_REG1));

	value0 = ((value0 >> 24) & 0xff) |
			 ((value0 << 8) & 0xff0000) |
			 ((value0 >> 8) & 0xff00) |
			 ((value0 << 24) & 0xff000000);

	value1 = ((value1 >> 24) & 0xff) |
			 ((value1 << 8) & 0xff0000) |
			 ((value1 >> 8) & 0xff00) |
			 ((value1 << 24) & 0xff000000);

	/************************************/

    printf("ddr_param_efuse_init reg0:%x\n",value0);
    ...
    if(true == is16bitdw){//16bit
        set_ddr_data(value0,value1,data,T40_EFUSE_PARAM_HAMM_NUBBIT_16BIT,is16bitdw);
        ret=check_HammingCode(data,T40_EFUSE_PARAM_DATA_NUBBIT_16BIT+T40_EFUSE_PARAM_INDEX_MODE_BIT,ddr3_param_data,is16bitdw);
    }else{//32bit
        set_ddr_data(value0,value1,data,T40_EFUSE_PARAM_HAMM_NUBBIT_32BIT,is16bitdw);
        ret=check_HammingCode(data,T40_EFUSE_PARAM_DATA_NUBBIT_32BIT+T40_EFUSE_PARAM_INDEX_MODE_BIT,ddr3_param_data,is16bitdw);
    }
