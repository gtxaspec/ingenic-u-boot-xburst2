...
{
	u32 tmp, cap_save, num_ports,scontrol;
	...
	for (i = 0; i < probe_ent->n_ports; i++) {
		...
		scontrol = readl(&(port_mmio->sctl));
		scontrol = (scontrol & 0x0f0) | 0x301;
		writel_with_flush(scontrol, &(port_mmio->sctl));
		sata_phy_reset(i, 0);
		writel_with_flush(scontrol, &(port_mmio->sctl));
		mdelay(1);
		scontrol = readl(&(port_mmio->sctl));
		do {
			...
			writel(scontrol, &(port_mmio->sctl));
			sata_phy_reset(i, 1);
			mdelay(200);
			scontrol = readl(&(port_mmio->sctl));
...
static void ahci_fill_cmd_slot(struct ahci_ioports *pp, u32 cmd_slot, u32 opts)
{
	...
	cmd_hdr->status = 0;
	cmd_hdr->tbl_addr = cpu_to_le32(pp->cmd_tbl & 0x7fffffff);
		...
		u8 *buf, u32 buf_len, s32 is_write)
{
	...
	//printf("cmd_slot=0x%08x\n",AHCI_GET_CMD_SLOT(readl(&(port_mmio->ci))));
	...
	if (buf && buf_len)
		sg_count = ahci_fill_sg(probe_ent, port, (unsigned char *)((u32)buf&0x7fffffff), buf_len);
	...
	invalid_dcache_range((int)(pp->cmd_slot),
	...
	if (!is_write){
		if(buf){
			invalid_dcache_range((ulong)buf, (ulong)buf+buf_len);
		}
	}
	flush_cache_all();
	return buf_len;
					...
					u8 port)
{
	...
	printf("Link up Port-%d(statue=0x%x)!\n",port,port_status);
	...
	writel_with_flush((u32)pp->cmd_slot - 0x80000000, &(port_mmio->clb));
	writel_with_flush(pp->rx_fis - 0x80000000, &(port_mmio->fb));
	...
	while ((readl(&(port_mmio->tfd)) & (SATA_PORT_TFD_STS_ERR |
		...
		mdelay(200);
	if (timeout <= 0) {
		printf("Device not ready for BSY, DRQ and"
			"ERR in TFD!\n");
		return -1;
	}
...
int init_sata(int dev)
{
	...
	for (i = 0; i < probe_ent->n_ports; i++) {
		if ((linkmap >> i) && ((linkmap >> i) & 0x01)) {
			...
			if(i == CONDIF_DWC_AHSATA_ACTIVE_PORT)
...
static u32 dwc_ahsata_rw_cmd_ext(int dev, u64 start, lbaint_t blkcnt,
				u8 *buffer, int is_write)
{
	struct ahci_probe_ent *probe_ent =
		(struct ahci_probe_ent *)sata_dev_desc[dev].priv;
...
u32 ata_low_level_rw_lba48(int dev, u64 blknr, lbaint_t blkcnt,
 ...
 */
ulong sata_read(int dev, u64 blknr, lbaint_t blkcnt, void *buffer)
{
	u32 rc;
...
ulong sata_write(int dev, u64 blknr, lbaint_t blkcnt, const void *buffer)
{
	u32 rc;
	struct ahci_probe_ent *probe_ent =
		(struct ahci_probe_ent *)sata_dev_desc[dev].priv;
