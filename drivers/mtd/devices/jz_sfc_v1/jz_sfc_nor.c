		...
		unsigned int addr_plus, unsigned int addr_len, unsigned int data_en)
{
	...
	tmp &= ~(TRAN_CONF0_ADDR_WIDTH_MSK | DMYBITS_MSK | TRAN_CONF0_CMD_MSK | TRAN_CONF0_PHASE_FORMAT | TRAN_CONF0_DATEEN);
	if (data_en) {
		tmp |= (addr_len << TRAN_CONF0_ADDR_WIDTH_OFFSET) | TRAN_CONF0_CMD_EN |
			TRAN_CONF0_DATEEN | (cmd << CMD_OFFSET);
	} else {
		tmp |= (addr_len << TRAN_CONF0_ADDR_WIDTH_OFFSET) | TRAN_CONF0_CMD_EN |
			(cmd << CMD_OFFSET);
	}
	...
	sfc_set_mode(0,TRAN_CONF1_SPI_QUAD);
...
void register_init(void)
{
	int n;
	for(n = 0; n < N_MAX; n++) {
		jz_sfc_writel(0, SFC_TRAN_CONF(n));
		jz_sfc_writel(0, SFC_DEV_ADDR(n));
		jz_sfc_writel(0, SFC_DEV_ADDR_PLUS(n));
	}

	jz_sfc_writel(0, SFC_DEV_CONF);
	jz_sfc_writel(0, SFC_DEV_STA_EXP);
	jz_sfc_writel(0, SFC_DEV_STA_MSK);
	jz_sfc_writel(0, SFC_TRAN_LEN);
	jz_sfc_writel(0, SFC_MEM_ADDR);
	jz_sfc_writel(0, SFC_TRIG);
	jz_sfc_writel(0, SFC_SCR);
	jz_sfc_writel(0, SFC_INTC);
	jz_sfc_writel(0, SFC_CGE);
	jz_sfc_writel(0, SFC_RM_DR);
}
...
int sfc_init(void )
{
	...
	for (i = 0; i < 6; i++) {
		...
		jz_sfc_writel(jz_sfc_readl(SFC_TRAN_CONF1(i))& (~(TRAN_CONF1_TRAN_MODE_MSK)),SFC_TRAN_CONF1(i));
	}
...
void sfc_set_quad_mode(void)
{
	...
	if(quad_mode != NULL){
		...
		if(quad_mode->otp_ops_mode)
			sfc_send_cmd(&cmd[4],0,0,0,0,0,1);
		...
		if(quad_mode->otp_ops_mode)
			sfc_send_cmd(&cmd[5],0,0,0,0,0,1);
		quad_mode_is_set = 1;
...
int sfc_nor_write(struct spi_flash *flash, unsigned int src_addr, unsigned int count,unsigned int dst_addr)
{
	...
	ret = jz_sfc_write(flash,src_addr,count,(const void *)dst_addr);
	if (ret) {
...
void sfc_for_nand_init(int sfc_quad_mode)
{
	...
	tmp &= ~(DEV_CONF_CMD_TYPE | DEV_CONF_CPHA | DEV_CONF_CPOL | DEV_CONF_SMP_DELAY_MSK |
				DEV_CONF_THOLD_MSK | DEV_CONF_TSETUP_MSK | DEV_CONF_TSH_MSK);
	tmp |= (DEV_CONF_CEDL | DEV_CONF_HOLDDL | DEV_CONF_WPDL | 1 << DEV_CONF_SMP_DELAY_OFFSET);
	jz_sfc_writel(tmp,SFC_DEV_CONF);
	for (i = 0; i < 6; i++) {
		jz_sfc_writel((jz_sfc_readl(SFC_TRAN_CONF(i))& (~(TRAN_CONF1_TRAN_MODE_MSK | TRAN_CONF0_PHASE_FORMAT))),SFC_TRAN_CONF(i));
	     if(sfc_quad_mode==1)
	     {
		unsigned int temp=0;
...
int sfc_do_nor_features(int argc, char * const argv[])
{
	...
	if(strcmp(argv[1], "r")  == 0){
		...
		sfc_send_cmd(&cmd[1],1,0,0,0,1,0);
		udelay(25000);
		sfc_read_data(&tmp, 1);
		tmp &= 0xff;
		printf("0x%02x\n",tmp);
	...
	if(strcmp(argv[1], "w")  == 0){
		...
		sfc_write_data(&tmp,WR_DATE_SIZE);
		return 0;
	}
	return -1;
}
