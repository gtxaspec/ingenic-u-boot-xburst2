...
int sfc_do_nor_features(int argc, char *const argv[])
{
    ...
    else if (strcmp(argv[1], "sfc") == 0)
    {
        if (argc != 10)
        {
            printf("dnor sfc read <cmd> <len_byte> <addr> <addr_len> <dummy_bit> <daten> <dir>\n\n");
            return -1;
        }
        else if (strcmp(argv[2], "read") == 0)
        {
            ...
            addr_len  = simple_strtoul(argv[6], NULL, 10);
            dummy_bit = simple_strtoul(argv[7], NULL, 10);
            daten     = simple_strtoul(argv[8], NULL, 10);
            dir       = simple_strtoul(argv[9], NULL, 10);
            dir       = 0;
            sfc_send_cmd(0, &cmd[1], len, addr, addr_len, dummy_bit, daten, dir);
            sfc_read_data(0, &data, len);
            printf("get data: \n");
