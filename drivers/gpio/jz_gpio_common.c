...
// lev:1~7
// PA0-22 and PC8-31 only 1-3lev, other 1~7lev。
void gpio_set_driver_strength(enum gpio_port n, unsigned int pins, unsigned char lev)
{
	...
	if (lev & (1 << 2))
		writel(pins, base + PXPDS2S);
	else
		writel(pins, base + PXPDS2C);

	if (lev & (1 << 1))
		writel(pins, base + PXPDS1S);
	else
		writel(pins, base + PXPDS1C);

	if (lev & 1)
		writel(pins, base + PXPDS0S);
	else
		writel(pins, base + PXPDS0C);

	if (n == GPIO_PORT_A && (pins &= 0x7FFFFF) && lev > 3)
	{
		writel(pins, base + PXPDS0S);
		writel(pins, base + PXPDS1S);
	}
	else if (n == GPIO_PORT_C && (pins &= 0xFFFFFF00) && lev > 3)
	{
		writel(pins, base + PXPDS0S);
		writel(pins, base + PXPDS1S);
	}
