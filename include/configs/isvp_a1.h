...
#elif defined APLL_1236M
#define CONFIG_SYS_APLL_FREQ		1236000000	/*If APLL not use mast be set 0*/
#define CONFIG_SYS_APLL_MNOD		((103 << 20) | (1 << 14) | (2 << 11) | (1<<8))
#elif defined APLL_1260M
#define CONFIG_SYS_APLL_FREQ		1260000000	/*If APLL not use mast be set 0*/
#define CONFIG_SYS_APLL_MNOD		((105 << 20) | (1 << 14) | (2 << 11) | (1<<8))
#elif defined APLL_1296M
#define CONFIG_SYS_APLL_FREQ		1296000000	/*If APLL not use mast be set 0*/
#define CONFIG_SYS_APLL_MNOD		((108 << 20) | (1 << 14) | (2 << 11) | (1<<8))
...
#define CONFIG_SYS_VPLL_MNOD		((100 << 20) | (1 << 14) | (2 << 11) | (1<<8))
#define CONFIG_SYS_EPLL_FREQ		1500000000	//If EPLL not use mast be set 0
#define CONFIG_SYS_EPLL_MNOD		((125 << 20) | (1 << 14) | (2 << 11) | (1<<8))
#define SEL_SCLKA		2
#define SEL_CPU			1
#define SEL_H0			2
#define SEL_H1			3
...
#define DIV_H1				(CONFIG_SYS_EPLL_FREQ / 500000000)
...
#define DDR_PHY_PARAMS_TEST_A1 //Edwin.Wen@Ingenic_20221212 used for A1 CHIP DDR parameters test.
*/
#define A1_CHIPID_THIRD_WORD (0xb3540208)

#if defined(CONFIG_A1N)
/* A1N */
#define CONFIG_DDR_TYPE_DDR3	1
#define CONFIG_DDR_BUSWIDTH_32	0	/* 1-32bit-width, 0-16bit-width */
#define CONFIG_DDR_DW32			0	/* 1-32bit-width, 0-16bit-width */
#define CONFIG_DDR3_W632GU6NG			/* 16 bit ddr3 2G(1*2G 16bit) */
#elif defined(CONFIG_A1L)
/* A1L */
#define CONFIG_DDR_TYPE_DDR3	1
#define CONFIG_DDR_BUSWIDTH_32	0	/* 1-32bit-width, 0-16bit-width */
#define CONFIG_DDR_DW32			0	/* 1-32bit-width, 0-16bit-width */
#define CONFIG_DDR3_W631GU6NG			/* 16 bit ddr3 1G(1*1G 16bit) */
#elif defined(CONFIG_A1NT)
/* A1NT */
#define CONFIG_DDR_TYPE_DDR3	1
#define CONFIG_DDR_BUSWIDTH_32	1	/* 1-32bit-width, 0-16bit-width */
...
#define CONFIG_DDR_BUSWIDTH_32	1	/* 1-32bit-width, 0-16bit-width */
#define CONFIG_DDR_DW32			1	/* 1-32bit-width, 0-16bit-width */
#define CONFIG_DDR3_W632GU6NG		/* 32 bit ddr3 2G(2*2G16bit) */
#elif defined(CONFIG_A1A)
/* A1A */
#define CONFIG_DDR_TYPE_DDR3	1
#define CONFIG_DDR_BUSWIDTH_32	1	/* 1-32bit-width, 0-16bit-width */
...
#define A1A_DDR3_8G
#ifdef A1A_DDR3_2G
#define CONFIG_DDR3_M15T2G16128A_2R             /* DDR3 2Gb */
#elif defined(A1A_DDR3_8G)
#define CONFIG_DDR3_M15T8G16512A_2S		/* DDR3 8Gb */
#endif
#else
#endif
...
/*Instead of using the following macro definitions,
kgd parameters are extracted from ddr3_param_t.*/
#if 0
#if 1
...
#elif defined(CONFIG_A1X)
#define CONFIG_DDR_DRIVER_OUT_STRENGTH_1 0
#if (A1X_DDR_VERSION == 1)
#define CONFIG_DDR_DRIVER_OUT_STRENGTH_0 1 //V1 SET 1
#else
 ...
 */
#if defined(CONFIG_A1X)
#define BOOTARGS_COMMON "console=ttyS1,115200n8 mem=254M@0x0 rmem=258M@0x2FE00000"
#else
...
#ifdef CONFIG_OF_LIBFDT
	#define CONFIG_BOOTARGS BOOTARGS_COMMON " init=/linuxrc rootfstype=squashfs root=/dev/mtdblock2 rw mtdparts=jz_sfc:384k(boot),2560k(kernel),3072k(root),64k(dtb),-(appfs) lpj=11968512 quiet"
#else
	#define CONFIG_BOOTARGS BOOTARGS_COMMON " init=/linuxrc rootfstype=squashfs root=/dev/mtdblock2 rw mtdparts=jz_sfc:384k(boot),2560k(kernel),3072k(root),-(appfs) lpj=11968512 quiet"
#endif
...
#elif defined(CONFIG_SFC_NOR)
#ifdef CONFIG_OF_LIBFDT
	#define CONFIG_BOOTARGS BOOTARGS_COMMON " init=/linuxrc rootfstype=squashfs root=/dev/mtdblock2 rw mtdparts=jz_sfc:384k(boot),2560k(kernel),3072k(root),64k(dtb),-(appfs) lpj=11968512"
#else
	#define CONFIG_BOOTARGS BOOTARGS_COMMON " init=/linuxrc rootfstype=squashfs root=/dev/mtdblock2 rw mtdparts=jz_sfc:384k(boot),2560k(kernel),3072k(root),-(appfs) lpj=11968512"
#endif
#elif defined(CONFIG_SFC_NAND)
...
#ifdef CONFIG_OF_LIBFDT
	#define CONFIG_BOOTCOMMAND "sf probe;sf read 0x80600000 0x60000 0x280000;sf read 0x83000000 0x560000 0x10000;bootm 0x80600000 - 0x83000000"
#else
	#define CONFIG_BOOTCOMMAND "sf probe;sf read 0x80600000 0x60000 0x280000;bootm 0x80600000"
#endif
...
#define CONFIG_DWC_AHSATA_BASE_ADDR SATA_BASE
#define CONDIF_DWC_AHSATA_ACTIVE_PORT (0)
#define CONFIG_LBA48
#define CONFIG_INGENIC_SATA_PHY
...
#ifdef CONFIG_SFC_NOR
#define CONFIG_CMD_TFTPDOWNLOAD     1 /* tftpdownload cmd support */
#endif
...
#define A1_GMAC0_PORT_INIT_FUNC GPIO_FUNC_0

#define A1_GMAC1_PORTS_GROUP GPIO_PORT_A
#define A1_GMAC1_PORT_PINS (0x7fff << 15)
#define A1_GMAC1_PORT_INIT_FUNC GPIO_FUNC_0

/*######################### UBOOT ETHERNET DTS CONFIG #########################

if use rmii 100Mbps net card,must define A1_MACxPHY_RMIIx_MACCDR.

1.  TUCANA BOARD

        #define CONFIG_ETH0MII              "rmii"
        #define A1_MAC0PHY_RMII0_MACCDR     A1_CPM_MACCDR_50MHZ
        #define A1_GMAC0_PHY_RESET_PIN	    GPIO_PC(19)

        #define CONFIG_ETH1MII              "rgmii"
        #define A1_GMAC1_PHY_RESET_PIN	    GPIO_PC(17)


2.  GORILLA BOARD

        #define CONFIG_ETH0MII        	    "rgmii"
        #define A1_GMAC0_PHY_RESET_PIN      GPIO_PC(19)
...
3.  CALF1.0 | CALF1.1 BOARD

        #define CONFIG_ETH0MII        	    "rmii"
        #define A1_MAC0PHY_RMII0_MACCDR     A1_CPM_MACCDR_50MHZ
        #define A1_GMAC0_PHY_RESET_PIN	    GPIO_PC(18)

        #define CONFIG_ETH1MII              "rgmii"
        #define A1_GMAC1_PHY_RESET_PIN	    GPIO_PC(19)


4.  CALF1.2 | CALF1.4 | CALF1.5 | CALF2.0 | CALF2.0.1 BOARD

        #define CONFIG_ETH0MII              "rmii"
        #define A1_MAC0PHY_RMII0_MACCDR     A1_CPM_MACCDR_25MHZ
        #define A1_GMAC0_PHY_RESET_PIN      GPIO_PC(18)
...
5.  CALF1.3 BOARD
        ...
        #define CONFIG_BOARD_CALF_1_3
        #define CONFIG_ETH0MII              "rgmii"
        #define A1_GMAC0_PHY_RESET_PIN      GPIO_PC(18)

        #define CONFIG_ETH1MII              "rmii"
        #define A1_MAC1PHY_RMII1_MACCDR     A1_CPM_MACCDR_50MHZ
        #define A1_GMAC1_PHY_RESET_PIN      GPIO_PC(19)
...
6.  CALF1.6 BOARD
        ...
        #define CONFIG_ETH1MII              "rmii"
        #define A1_MAC1PHY_RMII1_MACCDR     A1_CPM_MACCDR_25MHZ
        #define A1_GMAC1_PHY_RESET_PIN      GPIO_PC(19)


############################ UBOOT DTS CONFIG END ############################*/
/*
*  defualt config is CALF1.2 | CALF1.4 | CALF1.5 | CALF2.0 | CALF2.0.1 board.start.
*/
...
#ifndef A1_MAC0PHY_RMII0_MACCDR
#define A1_MAC0PHY_RMII0_MACCDR		A1_CPM_MACCDR_25MHZ
#endif
#ifndef A1_MAC1PHY_RMII1_MACCDR
#define A1_MAC1PHY_RMII1_MACCDR		A1_CPM_MACCDR_25MHZ
#endif
#endif//NET END
...
#define CONFIG_USB_DWC2_REG_ADDR  0x13600000
#define CONFIG_USB1_DWC2_REG_ADDR 0x13640000
#define CONFIG_USB2_DWC2_REG_ADDR 0x13680000
...
/*#define CONFIG_HARD_LZMA //默认硬件lzma解压*/
	...
	#define CONFIG_DECMP_HEAP_ADRS        	0x80300000
#endif
...
#else
#define CONFIG_SYS_MONITOR_LEN		(334 * 1024)
#endif
...
#else
#define CONFIG_SPL_MAX_SIZE		34816	/* 34816 = 34KB*/
#define CONFIG_SPL_PAD_TO		34816	/* equal to spl max size in T15g */
#endif
