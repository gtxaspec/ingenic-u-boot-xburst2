...
#define DDR_tCWL   (DDR_CL - 2)	/* DDR3 only: CAS Write Latency, 5 to 8 */
#endif

/*
 * DDR3 controller timing1 register
 */
#define DDR_tRAS DDR__ns(35)  /* tRAS: ACTIVE to PRECHARGE command period to the same bank. ns*/
...
#define DDR_tRC  DDR__ns(49)  /* ACTIVE to ACTIVE command period to the same bank. ns*/
#define DDR_tWR  DDR__ns(15)  /*FIXME WRITE Recovery Time defined by register MR of DDR2 memory, ns*/
#define DDR_tRRD MAX(DDR__tck(4), DDR__ps(7501)) /* FIXME ACTIVE bank A to ACTIVE bank B command period. DDR3 - tCK*/
...
#define DDR_tFAW   DDR__ns(40)	/* Four bank activate period, DDR3 - ns */
 ...
 */
#define DDR_tREFI   DDR__ns(3900)	/* Refresh period: 64ms / 32768 = 1.95 us , 2 ^ 15 = 32768 */
